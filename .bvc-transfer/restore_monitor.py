"""Transfer-only replay of the tested subprocess supervisor replacement."""
from pathlib import Path
import os
p=Path(os.environ['BVC_REPO'])/'big-video-converter/usr/share/big-video-converter/utils/conversion.py'
old=p.read_text(); errors=old[old.index('def _ffmpeg_error_map'):old.index('def detect_bit_depth_info')]
s='''"""Supervise conversion processes without guessing outputs or touching GTK off-thread."""
...
def run_with_progress_dialog(app, cmd: list, title_suffix, input_file=None,
    ...
    try:
        identity = FileIdentity.capture(source_file) if source_file else None
        if destination and os.path.lexists(destination):
            raise FileExistsError(f"Output already exists: {destination}")
        if cancel_event.is_set():
            return ConversionResult(False, -1, cancelled=True)
        ...
        call_on_main(failed)
        return ConversionResult(False, -1, error=str(error))
    ...
    try:
        ...
        if returncode:
            ...
            if progress_item.delete_original and source_file and not extraction_only:
                try:
                    if identity is None:
                        raise ValueError("Input metadata or identity could not be verified")
...
'''
s=s.replace('        while selector.get_map():','        while selector.get_map() or process.poll() is None:')
s=s.replace('        identity = FileIdentity.capture(source_file) if source_file else None','''        identity = None
        if source_file:
            ...
            try:
                ...
                if not os.path.isfile(source_file):
                    raise''')
s=s.replace('        if cancel_event.is_set():\n            return ConversionResult(False, -1, cancelled=True)','        if cancel_event.is_set():\n            raise InterruptedError("Conversion cancelled before starting")')
a=s.index('    try:\n        identity = None',s.index('def run_with_progress_dialog'));b=s.index('        def prepare_item():',a);c=s.index('        thread = threading.Thread(',b)
checks=s[a:b];prepare=s[b:c]
s=s[:a]+'    try:\n'+prepare+checks[len('    try:\n'):]+'''        call_on_main(setattr, progress_item, "process", process, wait=True)
'''+s[c:]
s=s.replace('''        def failed(message=str(error)):
            if counted:
                app.conversions_running = max(0, app.conversions_running - 1)
            app.show_error_dialog(_("Error starting conversion: {0}").format(message))
            if not is_segment_batch:
                app.conversion_completed(False, file_path=input_file, job_id=job_id)
        call_on_main(failed)
        return ConversionResult(False, -1, error=str(error))''','''        was_cancelled = isinstance(error, InterruptedError) or cancel_event.is_set()
        def failed(message=str(error)):
            if counted:
                app.conversions_running = max(0, app.conversions_running - 1)
...
s=s.replace('                    if identity is None:\n                        raise ValueError("Input metadata or identity could not be verified")','                    if identity is None or duration is None:\n                        raise ValueError("Input duration, metadata or identity could not be verified")')
p.write_text(s)
